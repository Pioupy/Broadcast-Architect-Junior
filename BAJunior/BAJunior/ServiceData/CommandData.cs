...
namespace BAJunior.ServiceData
{
    class CommandData
    {
        ...
        public List<Param> readParamByCommand(int id)
        {
            ...
            string requete = "SELECT p.IDParam, p.Name, p.Value, p.IsUser, p.IDCommand  FROM Command c, Param p WHERE c.IDCommand=p.IDCommand AND c.IDCommand=" + id + " order by c.IDCommand asc";
            try
            {
                DataTable reader = m_dbUtils.executeReader(requete);
